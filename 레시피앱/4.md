...
- [3. 과일 앱 이리저리 만지고, 고쳐보기](#3-과일-앱-이리저리-만지고-고쳐보기)
	...
	- [3.5. 여기까지 깃허브에 올리기](#35-여기까지-깃허브에-올리기)
...
[!button variant='success' icon='download' text='지난 수업 파일' target='blank'](./assets/04/start.zip)
==- 파일활용방법
...
- 서버에서 넘어온 데이터를 저장할 수 있도록 fruitILike라는 변수를 만든 다음 빈 배열을 할당하자.
...
```jsx #
const fruitDB = [
	{ name: '바나나 ', image: 'http://qwerew.cafe24.com/images/banana.png' },
	{ name: '오렌지', image: 'http://qwerew.cafe24.com/images/orange.png' },
	{ name: '사과', image: 'http://qwerew.cafe24.com/images/apple.png' },
	{  name: '메론', image: 'http://qwerew.cafe24.com/images/melon.jpg' },
];
...
- fruitILike에 있는 데이터로 컴포넌트를 여러 개 만들려면 자바스크립트 함수 map()의 사용 방법을 알아야 한다.
...
- 자 이제 크롬 브라우저에서 벗어나 VSCode 화면으로 돌아가서 ./src/App.js를 보자.
  그리고 fruitILike 배열을 다시 한번 눈으로 확인하면서 map 함수를 어떻게 적용할지 상상해 보자.
...
- foodILike.map(...)과 같이 작성하고, map() 함수에 전달할 인자에는 dish => <Fruit .../>와 같이 컴포넌트를 반환하는 함수를 전달하면 될 것이다.
  dish에는 배열에 있는 원소, 즉 객체 [name: '...', image: '...']이 하나씩 넘어올 것이다.
  이걸 dish.name, dish.image와 같은 방법으로 컴포넌트에 전달하면 된다.
  이제 천천히 코딩해 보자.
...
```jsx #
const fruitDB = [
	{ id: 1, name: '바나나', image: 'http://qwerew.cafe24.com/images/banana.png' },
	{ id: 2, name: '오렌지', image: 'http://qwerew.cafe24.com/images/orange.png' },
	{ id: 3, name: '사과', image: 'http://qwerew.cafe24.com/images/apple.png' },
	{ id: 4,  name: '메론', image: 'http://qwerew.cafe24.com/images/melon.jpg' },
];
```
...
- {fruitILike.map(dish => ( <Fruit key={dish.id} name={dish.name} picture={dish.image} /> ))}를 {fruitILike.map( renderFruit )}로 변경하자.
