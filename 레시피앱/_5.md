...
> ![](./assets/5-27.jpg)
>
> Plate 컴포넌트의 햄버거가 <mark>있는 상태에서 없는 상태로 바꾸면</mark> 된다. {.h3}
>
> 컴포넌트 내부의 데이터를 변경하는 것을  **상태관리** 라고 하며 상태관리를 이해하려면 우선 컴포넌트의 LifeCycle을 이해해야 한다. {.h3}
...
리액트에서는 컴포넌트의 생명주기를 관리하기 위해 함수형 컴포넌트와 클래스형 컴포넌트를 사용할 수 있습니다. 각각의 컴포넌트 유형은 다른 방식으로 생명주기를 관리합니다.

### 클래스형 컴포넌트의 생명주기

클래스형 컴포넌트에서는 생명주기 메서드를 사용하여 컴포넌트가 생성되고 업데이트되며 제거될 때 수행할 작업을 정의할 수 있습니다. 주요 생명주기 메서드는 다음과 같습니다:

1. **마운팅(Mounting)**
   - `constructor(props)`: 컴포넌트가 생성될 때 가장 먼저 실행되는 메서드입니다. 초기 상태를 설정할 수 있습니다.
   - `static getDerivedStateFromProps(props, state)`: props로 받아온 값을 state에 동기화시킬 때 사용합니다.
   - `render()`: 컴포넌트의 UI를 렌더링하는 메서드입니다.
   - `componentDidMount()`: 컴포넌트가 마운트된 직후 호출되며, 여기서 데이터 로딩 같은 비동기 작업을 처리할 수 있습니다.

2. **업데이트(Updating)**
   - `static getDerivedStateFromProps(props, state)`: 컴포넌트가 새로운 props를 받았을 때 호출됩니다.
   - `shouldComponentUpdate(nextProps, nextState)`: 컴포넌트가 리렌더링을 할지 말지 결정합니다. 기본적으로 true를 반환합니다.
   - `render()`: 컴포넌트를 리렌더링합니다.
   - `getSnapshotBeforeUpdate(prevProps, prevState)`: 업데이트가 일어나기 바로 직전의 DOM 상태를 가져옵니다.
   - `componentDidUpdate(prevProps, prevState, snapshot)`: 컴포넌트의 업데이트가 끝난 후 호출됩니다.

3. **언마운팅(Unmounting)**
   - `componentWillUnmount()`: 컴포넌트가 DOM에서 제거될 때 호출됩니다. 여기서 필요한 정리 작업을 수행할 수 있습니다.


### 함수형 컴포넌트의 생명주기

함수형 컴포넌트에서는 `useState`, `useEffect` 등의 Hook을 사용하여 생명주기를 관리합니다.

1. **마운팅(Mounting) 및 언마운팅(Unmounting)**
   - `useEffect(() => { ... }, [])`: 배열 내의 의존성이 없기 때문에 컴포넌트가 마운트될 때 한 번만 실행됩니다. 반환되는 클린업 함수는 컴포넌트가 언마운트될 때 실행됩니다.

2. **업데이트(Updating)**
   - `useEffect(() => { ... })`: 의존성 배열이 없는 경우, 컴포넌트가 업데이트될 때마다 실행됩니다.
   - `useEffect(() => { ... }, [deps])`: 의존성 배열에 지정된 값이 변경될 때마다 실행됩니다.

함수형 컴포넌트는 Hook을 통해 더 간결하고 직관적인 방식으로 생명주기를 관리할 수 있으며, 클래스형 컴포넌트보다 선호되는 경향이 있습니다. 그러나 복잡한 상태 관리나 생명주기 메서드가 필요한 경우에는 여전히 클래스형 컴포넌트가 유용할 수 있

이런 자료를 참고했어요.
[1] velog - React 클래스형 컴포넌트 생명주기(Lifecycle) (https://velog.io/@wlwl99/React-%ED%81%B4%EB%9E%98%EC%8A%A4%ED%98%95-%EC%BB%B4%ED%8F%AC%EB%84%8C%ED%8A%B8-%EC%83%9D%EB%AA%85%EC%A3%BC%EA%B8%B0Lifecycle)
[2] Contributor9 - [React] 함수형 컴포넌트 생명주기(lifecycle) 이해하기 (https://adjh54.tistory.com/43)
[3] Contributor9 - [React] 클래스 컴포넌트 생명주기(lifecycle) 이해하기 (https://adjh54.tistory.com/42)
[4] 티스토리 - [React] 함수형 컴포넌트의 생명주기(Life Cycle) (https://yooneeee.tistory.com/45)
