...
- [1. 시작파일](#1-시작파일)
- [2. Component State(상태관리)](#2-component-state상태관리)
	- [2.1. 클래스형 vs 함수형 상태관리](#21-클래스형-vs-함수형-상태관리)
	- [2.2. 함수형 컴포넌트의 상태관리](#22-함수형-컴포넌트의-상태관리)
		- [2.2.1. useState()](#221-usestate)
			- [2.2.1.1. 카운터를 만들어보자-1](#2211-카운터를-만들어보자-1)
			- [2.2.1.2. 카운터를 만들어보자-2](#2212-카운터를-만들어보자-2)
			- [2.2.1.3. 카운터를 만들어보자-3](#2213-카운터를-만들어보자-3)
			- [2.2.1.4. 카운터를 개선 해보자](#2214-카운터를-개선-해보자)
	- [2.3. 퀴즈퀴즈](#23-퀴즈퀴즈)
- [3. Component LifeCycle(생명주기)](#3-component-lifecycle생명주기)
	- [3.1. 클래스형 컴포넌트와 생명주기](#31-클래스형-컴포넌트와-생명주기)
		- [3.1.1. 클래스형 컴포넌트](#311-클래스형-컴포넌트)
		- [3.1.2. 클래스형 컴포넌트 생명주기](#312-클래스형-컴포넌트-생명주기)
	- [3.2. 함수형 컴포넌트의 생명주기](#32-함수형-컴포넌트의-생명주기)
		- [3.2.1. useEffect](#321-useeffect)
			- [3.2.1.1. 무한루프 문제상황 만들기](#3211-무한루프-문제상황-만들기)
			- [3.2.1.2. Mount-컴포넌트 렌더링 직전 딱 한번 실행](#3212-mount-컴포넌트-렌더링-직전-딱-한번-실행)
			- [3.2.1.3. Update-컴포넌트 렌더시](#3213-update-컴포넌트-렌더시)
			- [3.2.1.4. UnMount-컴포넌트가 사라질때 실행](#3214-unmount-컴포넌트가-사라질때-실행)
- [4. 관련영상링크](#4-관련영상링크)
	- [4.1. 리액트의 이벤트](#41-리액트의-이벤트)
	- [4.2. 조건부렌더링](#42-조건부렌더링)
- [5. 여기까지 깃허브에 올리기](#5-여기까지-깃허브에-올리기)
- [6. 완료파일](#6-완료파일)
...
## 2. Component State(상태관리)

### 2.1. 클래스형 vs 함수형 상태관리
...
##### 3.2.1.2. Mount-컴포넌트 렌더링 직전 딱 한번 실행
...
4. `return`문 내에 작성한다.
   ...
   ...
   ```
5. 실행화면에서 stop 클릭시 화면은 업데이트 되나 콘솔메시지는 계속 진행 되고 있다.
   ![](./assets/5-26.jpg){.w70}
6. 이제 정리함수를 작성한다. `UnMount` 컴포넌트의 useEffect 함수에 작성해야 한다.
   ...
   ```
7. 이제 stop 버튼을 클릭하면 실행화면과 콘솔메시지가 모두 종료 되었다.

## 4. 관련영상링크

### 4.1. 리액트의 이벤트

[!button variant='ghost' icon='link' text='이벤트' target='blank'](../리액트좀해라/4.md)

### 4.2. 조건부렌더링

[!button variant='ghost' icon='link' text='동적스타일&조건부렌더링' target='blank'](../리액트좀해라/7.md)
...
[!button variant='success' icon='download' text='유튜브강의 완료 파일' target='blank'](./assets/05/05_fin.zip)